...
-- Este script já foi executado em 2026-09-21 21:44 (11 contas de teste) e fica só como documento do que foi feito.
...
--| --
--| -- Alvo: TODA conta de auth.users exceto as 3 da equipe (<EMAIL_ADMIN>,
--| -- contato+personal@ybytu.app, contato+nutri@ybytu.app) = 10 alunos de teste + a conta
--| -- <EMAIL_STAFF_TESTE_UI_0919> (papel staff revogado, sem perfil) = 11 contas.
...
--|   where email not in ('<EMAIL_ADMIN>','contato+personal@ybytu.app','contato+nutri@ybytu.app');
...
--|   end if;
--|   if (select count(*) from auth.users u where u.email in ('<EMAIL_ADMIN>','contato+personal@ybytu.app','contato+nutri@ybytu.app')
